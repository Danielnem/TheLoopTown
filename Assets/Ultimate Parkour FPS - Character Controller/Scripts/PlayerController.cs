...
namespace UltimateParkourFPS
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        [Header("Camera")]
        [Tooltip("player main camera component")]
        [SerializeField] private Camera mainCamera;
        [Tooltip("player weapons camera component")]
        ...
        private static int lookXLimit = 90; // player upward/downward rotation limit
        ...
        [HideInInspector] public bool isGrounded; // if currently touching the ground
        ...
        private void SetFov()
        {
            ...
            if (isRunning) // if running
                newFOV += runFovIcrease;

            if (isSliding) // if sliding
                newFOV += slideFovIncrease;
        ...
        private void SetPlayerHeight()
        {
            if (isSliding || isCrouching) // if sliding or crouching
            {
                if (!changedPlayerHeight) // if height not changed yet
                {
                    ...
                    mainCamera.transform.localPosition /= 4f;
                }
            }
            else
            {
        ...
        private IEnumerator Slide(bool retry = false)
        {
            if (!isSliding) // if not already sliding
            {
                if (isGrounded && !touchingWallRight && !touchingWallLeft) // if touching a flat surface
                {
                    ...
                    soundPlayer.PlaySound(slidingSound); // play sliding sound
        ...
        private void Jump(bool retry = false)
        {
            if ((isGrounded || hasDoubleJump) && (Time.time >= (lastJumpTime + jumpBufferTime + 0.1f))) // if player is touching the ground or able to double jump
            {
                ...
                // play jump sound
                soundPlayer.PlaySound(jumpSound, volume: 0.6f);
        ...
        private IEnumerator CheckLandingSound()
        {
            ...
            // play landing sound
            soundPlayer.PlaySound(landingSound, volume: 0.6f);
        }
        #endregion
        ...
        private void CheckFootstepSound(float movementAmount)
        {
            ...
            {
                if (!soundPlayer.isPlaying || (soundPlayer.clip != jumpSound && soundPlayer.clip != landingSound)) // if sound is not already playing
                {
                    if (isRunning || isWallrunning) // if running
                    {
                        if (soundPlayer.clip != runningSound || !soundPlayer.isPlaying) // if not already playing running sound
                            soundPlayer.PlaySound(runningSound, loop: true); // play running sound
                    }
                    else if (soundPlayer.clip != walkingSound || !soundPlayer.isPlaying) // walking and not already playing walking sound
                        soundPlayer.PlaySound(walkingSound, loop: true); // play walking sound
                }
            }
            else if (soundPlayer.isPlaying && (soundPlayer.clip == runningSound || soundPlayer.clip == walkingSound)) // player stopped moving
        ...
        private void SetRotation()
        {
            ...
            mainCamera.transform.localRotation = Quaternion.Euler(currRotationX, 0, currentCameraLean);
